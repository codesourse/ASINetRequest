...
- (id)initWithURL:(NSURL *)newURL
   KeysAndObjects:(id            )item, ... NS_REQUIRES_NIL_TERMINATION
{
    return nil;
}
...
{
    if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		return [NSJSONSerialization JSONObjectWithData:[ASIDataDecompressor uncompressData:[self rawResponseData] error:NULL] options:NSJSONReadingMutableLeaves error:nil];
	} else if([self rawResponseData]){
		return [NSJSONSerialization JSONObjectWithData:[self rawResponseData] options:NSJSONReadingMutableLeaves error:nil];
	}
	return nil;
}
