   ...
   KeysAndObjects:(id            )item, ... NS_REQUIRES_NIL_TERMINATION;
{
    ...
    {                                   // so we'll handle it separately.
        ...
        [itemArray addObject:eachObject==nil?@"":eachObject]; // that isn't nil, add it to self's contents.
        va_end(argumentList);
    }
    NSAssert([itemArray count]%2!=1,
             @"\n----------------------------------------------------------------------\n\n入参有误%@\n----------------------------------------------------------------------\n",
             itemArray);
    ...
    {
        [postDictionary setValue:[itemArray objectAtIndex:i*2+1] forKey:[itemArray objectAtIndex:i*2]];
    }
    return [self initWithURL:newURL];
}
